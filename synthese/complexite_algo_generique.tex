Nous nous intéressons à présent à la complexité de l'algorithme générique (algo
...
On peut directement déduire de ce lemme lors de, pour un noeud actif $i$, il existe toujours un
noeud de son voisinage $j$ tel que $d(j) > d(i)$ et donc que lors d'un réétiquetage, la
minimisation de la hauteur des voisins ne se fait pas sur un ensemble vide.
...
\textbf{Preuve}\\
Une opération de réétiquetage n'affecte que la distance du noeud $i$ au noeud puits, par conséquent
